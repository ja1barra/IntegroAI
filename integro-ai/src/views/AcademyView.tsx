...
const COURSES = [
  { title: 'Outbound Mastery', desc: 'Build high-converting outbound sequences with AI assistance', lessons: 8, badge: '⚡' },
  { title: 'Demand Generation Playbook', desc: 'Turn MQLs into pipeline with automated nurture flows', lessons: 6, badge: '📣' },
  { title: 'Customer Success at Scale', desc: 'Predict churn and expand accounts using health signals', lessons: 7, badge: '💎' },
  { title: 'AI-Powered Growth', desc: 'Leverage IntegroAI agents to run full-funnel GTM motions', lessons: 10, badge: '🧠' },
]
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 16 }}>
        {COURSES.map(c => (
          <div key={c.title} style={{ background: 'rgba(255,255,255,0.6)', borderRadius: 'var(--radius)', border: '1px solid rgba(255,255,255,0.7)', padding: '20px 22px', backdropFilter: 'blur(12px)', opacity: 0.72, cursor: 'not-allowed' }}>
            <div style={{ fontSize: 28, marginBottom: 12 }}>{c.badge}</div>
